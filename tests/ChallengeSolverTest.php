<?php

namespace PouleR\SpotifyLogin\Tests;

use PHPUnit\Framework\TestCase;
use PouleR\SpotifyLogin\Exception\SpotifyLoginException;
use PouleR\SpotifyLogin\ChallengeSolver;

/**
 * Class ChallengeSolverTest
 */
class ChallengeSolverTest extends TestCase
{
    private ChallengeSolver $challengeSolver;

    /**
     *
     */
    public function setUp(): void
    {
        $this->challengeSolver = new ChallengeSolver();
    }

    /**
     *
     */
    public function testSolveChallenge(): void
    {
        $loginContext = hex2bin('');
        $hashCashPrefix = hex2bin('48859603d6c16c3202292df155501c55');

        $result = $this->challengeSolver->solveChallenge($loginContext, $hashCashPrefix, 10);

        $suffix = [0x7f, 0x7e, 0x55, 0x8b, 0xd1, 0x0c, 0x37, 0xd2, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x02, 0xc7];
        self::assertEquals($suffix, $result->getSuffix());
        self::assertEquals(711, $result->getIterations());
        self::assertGreaterThan(0, $result->getDuration());
    }

    /**
     *
     */
    public function testSolveChallengeMaxIterations(): void
    {
        $loginContext = hex2bin('');
        $hashCashPrefix = hex2bin('48859603d6c67c3202292df155501c88');

        $this->expectException(SpotifyLoginException::class);
        $this->expectExceptionMessage('Too many iterations needed to solve hashCash, try again');
        $this->challengeSolver->solveChallenge($loginContext, $hashCashPrefix, 10);
    }

    /**
     *
     */
    public function testSolveChallengeException(): void
    {
        $this->expectException(SpotifyLoginException::class);
        $this->expectExceptionMessage('Invalid hashCash length');
        $this->challengeSolver->solveChallenge(hex2bin('0000'), hex2bin('0000'), 8);
    }
}
